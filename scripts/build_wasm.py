...
def _wasm_init():
    ...
    Falls back to a temporary directory when /home/pyodide/ is not writable
    (e.g. during marimo export html-wasm validation on the build host).
    """
    import sys
    import tempfile
    from pathlib import Path

    # Prefer /home/pyodide (Pyodide MEMFS); fall back to a temp dir on the host.
    try:
        _HOME = Path("/home/pyodide")
        _HOME.mkdir(parents=True, exist_ok=True)
        (_HOME / ".write_test").touch()
        (_HOME / ".write_test").unlink()
    except (PermissionError, OSError):
        _HOME = Path(tempfile.mkdtemp(prefix="vault_wasm_"))
...
def _notebook_header() -> str:
    try:
        _mv = importlib.metadata.version("marimo")
    except importlib.metadata.PackageNotFoundError:
        _mv = "0.1.0"
    return (
        'import marimo\n\n'
        f'__generated_with = "{_mv}"\n'
